...
class PurePersuit:
	def __init__(self):
		...
		rospy.Subscriber('/dakebot/center_pose', PoseStamped, self.pose_cb, queue_size = 1)
	...
	def loop(self):
		...
		while not rospy.is_shutdown():
			if self.currentPose and self.currentWaypoints:
	...
	def calculateTwistCommand(self):
		...
		if(l > 0.02) and (alpha > -0.26 and alpha < 0.26):
			theta = math.atan(2 * 0.224 * math.sin(alpha) / l)
			twistCmd = Twist()
